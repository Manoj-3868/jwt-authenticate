...
import (
	...
	helpers "golang-jwt/Helpers"
	models "golang-jwt/Models"
	"log"
	"net/http"
	"strconv"
	"time"
...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		count1, err := userCollection.CountDocuments(ctx, emailFilter)
		...
		count, err := userCollection.CountDocuments(ctx, phoneFilter)
		if err != nil {
			log.Panic(err)
...
func Login() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		token, refreshToken, _ := helpers.GenerateAllTokens(*foundUser.Email, *foundUser.First_name, *foundUser.Last_name, foundUser.User_id, foundUser.User_id)
